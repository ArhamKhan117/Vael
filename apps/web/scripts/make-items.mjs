...
async function main() {
  // The catalogue only, by default. The item artwork under apps/web/public/items is drawn by an
  // image model now (see public/game/CREDITS.md), and this script's plate-and-icon composites would
  // silently overwrite eleven drawn images with pixel plates. `--art` is still here because the
  // composites are the fallback if the generated set is ever lost.
  if (process.argv.includes("--art")) {
    ...
    console.log("catalogue only; pass --art to redraw the pixel plates over the generated artwork")
  }
